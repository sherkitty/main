...
# a wallet connected to the first local online sherkittyd
N_WALLETS = 5
...
wallet_base = [builddir + "/bin/sherkitty-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_SHERKITTYDS):
  command_lines.append([str(18180+i) if x == "sherkittyd_rpc_port" else str(18280+i) if x == "sherkittyd_p2p_port" else str(18380+i) if x == "sherkittyd_zmq_port" else builddir + "/functional-tests-directory/sherkittyd" + str(i) if x == "sherkittyd_data_dir" else x for x in sherkittyd_base])
