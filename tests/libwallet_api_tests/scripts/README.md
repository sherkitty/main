...
* Running sherkitty node, linked to private/public testnet. 
  By default, tests expect daemon running at ```localhost:20770```,
  can be overridden with environment variable ```TESTNET_DAEMON_ADDRESS=<your_daemon_address>```
  [Manual](https://github.com/sherkittyexamples/private-testnet) explaining how to run private testnet.
...
* Directory with pre-generated wallets
  ...
  By default, tests expect these wallets to be in ```/var/sherkitty/testnet_pvt```. 
