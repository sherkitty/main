# Anonymity Networks with Sherkitty

Currently only Tor and I2P have been integrated into Sherkitty. The usage of
...
is made or until `sherkittyd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `sherkitty-wallet-cli` and
`sherkitty-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `sherkittyd` the ip/port of a socks proxy provided by a
...
which tells `sherkittyd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `sherkittyd` the inbound address, network
...
which tells `sherkittyd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `sherkittyd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `sherkittyd` localhost port 30000.
...
`sherkittyd` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 14770 for mainnet) instead of the p2p port:

```
HiddenServiceDir /var/lib/tor/data/sherkitty
HiddenServicePort 14770 127.0.0.1:14770
```
...
This will store key information in `/var/lib/tor/data/sherkitty` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/sherkitty/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Sherkitty network should have
similar timestamps.
...
### Intermittent Sherkitty Syncing

If a user only runs `sherkittyd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `sherkittyd` as often as possible to conceal when transactions are being sent.
...
`sherkittyd` currently selects two outgoing connections every 5 minutes for
...
`sherkittyd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `sherkittyd` should likely disconnect
