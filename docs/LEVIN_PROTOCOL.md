...
used by Sherkitty. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Sherkitty's
...
> This document does not currently list all data being sent by the sherkitty
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about Sherkitty p2p traffic!


## Header
This header is sent for every Sherkitty p2p message.
...
Sherkitty when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Sherkitty specific
...
### Requests
Requests are the basis of the admin protocol for Sherkitty. The `Q` bit must be
