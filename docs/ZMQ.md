# The Current/Future Status of ZMQ in Sherkitty

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Sherkitty
...
matching supported by both Sherkitty and ZMQ. The `format`, `context` and `event`
...
The Sherkitty daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Sherkitty daemon also ensures that
...
The Sherkitty daemon will send a `txpool_add` pub exactly once for each
